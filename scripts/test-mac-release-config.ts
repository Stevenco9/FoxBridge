...
assert.ok(workflow.includes('python -m pip install --upgrade setuptools'))
assert.ok(workflow.includes('import distutils'))
assert.ok(workflow.includes('PYTHON="$(which python)"'))
assert.ok(workflow.includes('npm_config_python'))
{
  const pythonIdx = workflow.indexOf('actions/setup-python@v5')
  const setuptoolsIdx = workflow.indexOf('python -m pip install --upgrade setuptools')
  const distutilsIdx = workflow.indexOf('import distutils')
  const pythonExportIdx = workflow.indexOf('PYTHON="$(which python)"')
  ...
  assert.ok(pythonExportIdx >= 0 && pythonExportIdx < npmCiIdx, 'PYTHON must be exported from which python before npm ci')
}
