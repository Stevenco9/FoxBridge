...
|-----------|--------|-------|
| Packaged Desktop public Cloud config | **CI FAIL-CLOSED** (Sprint 24.4A) / **LIVE-VALIDATED** (0.1.4 installed on both test Macs) | Production Mac/Windows builds require GitHub Actions Variables `FOXBRIDGE_CLOUD_URL`, `FOXBRIDGE_CLOUD_PUBLISHABLE_KEY`, `FOXBRIDGE_SCANNER_URL`; compiled `dist-electron` is checked before packaging |
